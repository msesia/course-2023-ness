...
from mfpi.utils import ClassificationDataset
...
class NNet(nn.Module):
    ...
    """
    def __init__(self, num_features, num_classes, num_hidden=64, dropout=0.1):
        """ Initialization
        Parameters
        ----------
...
class Net:
    """ Fit a neural network (classification) to training data
    """
    def __init__(self, num_features, num_classes, dropout=0.2, learning_rate=0.001,
        ...
        # Define NNet model
        self.model = NNet(self.num_features, self.num_classes, num_hidden=num_hidden, dropout=dropout)
    ...
    def fit(self, X, Y, return_loss=False):
        ...
        if self.calibrate>0:
            ...
            for b in range(self.calibrate):
                ...
                val_dataset = ClassificationDataset(X_valid, Y_valid)
    ...
    def predict_proba(self, X_test):
        """ Estimate the label probabilities given the features
        Parameters
        ----------
        x : numpy array of training features (nXp)
        Returns
        -------
        ret_val : numpy array of estimated probabilities (n,K)
        """
        ...
        prob = 1e-6 + np.concatenate(y_proba_list)
